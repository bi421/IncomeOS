...
def _extract_company(raw_data: str | dict | None) -> str:
    if raw_data is None:
        return "Unknown"
    ...
    if not isinstance(data, dict):
        return "Unknown"
    for key in ["company_name", "company", "Company", "employer", "Organization", "organization"]:
...
def _extract_description(raw_data: str | dict | None) -> str:
    if raw_data is None:
        return ""
    if isinstance(raw_data, str):
        try:
    ...
    if not isinstance(data, dict):
        return ""
    for key in ["description", "Description", "desc", "job_description", "jobDescription"]:
        if key in data and data[key]:
            val = data[key]
            if isinstance(val, str):
...
# ✅ ШИНЭ: get_jobs_by_skill – ажлын байрны жагсаалтыг буцаах
def get_jobs_by_skill(skill: str, limit: int = 10, db_path: Path | None = None) -> list[dict[str, Any]]:
    ...
    for row in rows:
        d = dict(row)
        raw = d.get('raw_data')
        d['company'] = _extract_company(raw)
        d['description'] = _extract_description(raw)
        result.append(d)
    return result
